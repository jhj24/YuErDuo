...
public class DrivingRouteOverLay extends RouteOverlay {
    ...
    private void colorWayUpdate(List<TMC> tmcSection) {
        ...
        addPolyLine(new PolylineOptions().add(startPoint,
                new LatLng(tmcSection.get(0).getPolyline().get(0).getLatitude(), tmcSection.get(0).getPolyline().get(0).getLongitude()))
                .setDottedLine(true));
        String status = "";
        for (int i = 0; i < tmcSection.size(); i++) {
            ...
            if (status.equals(segmentTrafficStatus.getStatus())) {
                for (int j = 1; j < mployline.size(); j++) {//第一个点和上一段最后一个点重复，这个不重复添加
                    mPolylineOptionscolor.add(new LatLng(mployline.get(j).getLatitude(), mployline.get(j).getLongitude()));
                }
            } else {
            ...
            if (i == tmcSection.size() - 1 && mPolylineOptionscolor != null) {
                addPolyLine(mPolylineOptionscolor.color(getcolor(status)));
                addPolyLine(new PolylineOptions().add(new LatLng(mployline.get(mployline.size() - 1).getLatitude(),mployline.get(mployline.size() - 1).getLongitude()), endPoint)
                        .setDottedLine(true));
            }
        }
    }
    ...
    private int getcolor(String status) {
        ...
        if (status.equals("畅通")) {
            return Color.parseColor("#38a800");
