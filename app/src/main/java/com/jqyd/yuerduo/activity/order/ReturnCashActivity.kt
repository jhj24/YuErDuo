 ...
 */
class ReturnCashActivity : CommonListActivity<ReturnCashBean>() {
    private var type: Int = 0
    private var mTitle: String = "返现券"
    ...
    override val url: String
        get() = URLConstant.GET_RETURN_CASH_LIST

    override val adapter: CommonDataListAdapter<ReturnCashBean, out RecyclerView.ViewHolder>
        get() = ReturnCashAdapter(this)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val adapter = adapterLocal as ReturnCashAdapter
        ...
        if (type == 0) {
            ...
            topBar_right_button.onClick { startActivity<ReturnCashActivity>("type" to 1) }
        } else {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == 1000 && resultCode == Activity.RESULT_OK) {
            val bean = data?.getSerializableExtra("data") as? ReturnCashBean
            val intent = Intent()
    ...
    fun setOnItemClicked(bean: ReturnCashBean) {
        if (allGoodsMoney >= bean.limitAmount && System.currentTimeMillis() in bean.startTimeDate..bean.endTimeDate) {
            startActivityForResult<CashCouponsGoodsListActivity>(1000, "data" to bean, "allGoodsMoney" to allGoodsMoney)
