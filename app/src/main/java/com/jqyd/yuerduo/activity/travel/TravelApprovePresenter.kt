...
class TravelApprovePresenter(activity: Activity, val billDefine: BillDefineX?, travelBean: TravelBean?) : BasePresenter(activity) {
    ...
    init {
        ...
        val bill = bill("travelApproveBill")
        if (bill == null) {
        ...
        if (0 != travelBean?.examinePermissions ?: 0 and 1) {
            operationList += ID_VALUE("0", "同意")
        }
        if (0 != travelBean?.examinePermissions ?: 0 and 2) {
            operationList += ID_VALUE("1", "驳回")
        }
        if (0 != travelBean?.examinePermissions ?: 0 and 4) {
            operationList += ID_VALUE("2", "转发")
        }
        if (0 == travelBean?.examinePermissions ?: 0) {
        ...
        button("commit")?.onClick {
            ...
            httpCall = commitBillX(activity, URLConstant.CHECK_ASK_TRAVEL, billData, params, object : GsonProgressHttpCallback<BaseBean>(activity, "正在提交") {
                override fun onSuccess(result: ResultHolder<BaseBean>) {
                    super.onSuccess(result)
                    EventBus.getDefault().post(RefreshNumberEvent(FunctionName.travel, "1"))
                    activity.toast("提交成功")
