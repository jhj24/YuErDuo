 ...
 */
class ChannelTypeTreeAdapter(activity: ChannelTypeTreeActivity) : RecyclerView.Adapter<ChannelTypeTreeAdapter.ItemViewHolder>() {

    var dp10 = UIUtils.dip2px(activity, 10f)
    var dataList = mutableListOf<QuDaoType>()
    var list = mutableListOf<QuDaoType>()
    var selectId = String()
    ...
    override fun getItemViewType(position: Int): Int {
        var type: Int = 0
        if (dataList.size > 0) {
            if (dataList[position].quDaoTypeList.size > 0) {
                type = 1
            } else {
                type = 0
            }
    ...
    override fun onBindViewHolder(holder: ItemViewHolder, position: Int) {
        ...
        with(holder.itemView) {
            ...
            if (bean.quDaoTypeList != null && bean.quDaoTypeList.size > 0) {
    ...
    inner class ItemViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        private var bean: QuDaoType? = null

        init {
            itemView.onClick {
                val bean = itemView.tag as QuDaoType
                this.bean = bean
                if (bean.quDaoTypeList != null && bean.quDaoTypeList.size > 0) {
                    ...
                    if (bean.isShowChildren) {
                        ...
                        TreeHandleUtil.sortData(bean.quDaoTypeList)
                        dataList.addAll(location + 1, bean.quDaoTypeList)
                        notifyItemRangeInserted(location + 1, bean.quDaoTypeList.size)
                    }
