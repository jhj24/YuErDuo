 ...
 */
class ChannelTypeTreeActivity : BaseActivity() {
    lateinit var adapter: ChannelTypeTreeAdapter
    lateinit var selectId: String
    lateinit var character: CharacterUtil

    lateinit var dataList: ArrayList<QuDaoType>
    lateinit var baseBeanList: ArrayList<QuDaoType>
    lateinit var listAdapter: ChannelTypeAdapter
    ...
    private fun initView() {
        ...
        adapter = ChannelTypeTreeAdapter(this)
        recyclerView.adapter = adapter

        rv_search.layoutManager = LinearLayoutManager(this)
        listAdapter = ChannelTypeAdapter()
    ...
    private fun onClick() {
        val intent = Intent()
        val list = adapter.list.filter { it.id.toString() == adapter.selectId }
        val item = if (list.isNotEmpty()) list[0] else null
        if (item == null) {
            toast("请选择客户类型")
            return
        }
        intent.putExtra("channelType", item)
    ...
    private fun filterData(text: String) {
        val filterDataList = ArrayList<QuDaoType>()
        if (text.isEmpty()) {
            ...
            filterDataList.clear()
            baseBeanList.filterTo(filterDataList) { it.quDaoTypeList.size == 0 && TreeHandleUtil.isFilter(character,text,it.name) }
    ...
    fun initData() {
        ...
        HttpCall.request(this, URLConstant.STORE_TYPE_URL, null, object : GsonDialogHttpCallback<QuDaoType>(this@ChannelTypeTreeActivity, "正在加载数据...") {
            override fun onSuccess(result: ResultHolder<QuDaoType>) {
                super.onSuccess(result)
                if (result.dataList.size > 0) {
                    val list = ArrayList<QuDaoType>()
                    result.dataList.forEach {
                        if (it.quDaoTypeList.size > 0) {
                            list.add(it)
                    ...
                    list.forEach {
                        ...
                        it.childrenSize = if (it.quDaoTypeList != null && it.quDaoTypeList.size > 0) 1 else 0
                    }
                    TreeHandleUtil.sortData(result.dataList)
                    baseBeanList = list
                    dataList = result.dataList as ArrayList<QuDaoType>
                    adapter.list = list
